...
class StitchingPipeline:
    ...
    @staticmethod
    def overlay_seam(image, seam_mask, color=(0, 0, 255), dilate_radius=2):
        if seam_mask is None or image is None:
            return image
        seam = seam_mask.astype(np.uint8)
        if dilate_radius > 0:
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (dilate_radius * 2 + 1, dilate_radius * 2 + 1))
            seam = cv2.dilate(seam, kernel)
        overlay = image.copy()
        overlay[seam > 0] = color
        return overlay
