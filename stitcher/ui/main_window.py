...
class StitchingGUI:
    def __init__(self, root):
        ...
        self.seam_overlay_image = None
        self.show_seam = tk.BooleanVar(value=False)
        self.thumbnail_refs = []
    ...
    def _create_widgets(self):
        ...
        self.show_seam_check = tk.Checkbutton(
            self.params_frame, text="显示缝合线",
            variable=self.show_seam,
            command=self._on_show_seam_changed,
            bg=P_MID, fg=P_TEXT,
            selectcolor=P_MID,
            activebackground=P_MID, activeforeground=GOLD_DARK,
            font=("Microsoft YaHei UI", 9))
    ...
    def _on_show_seam_changed(self):
    ...
    def _layout_widgets(self):
        ...
        self.show_seam_check.grid(row=6, column=0, columnspan=2, sticky=tk.W, pady=(10, 2))

        self.params_divider.grid(row=7, column=0, columnspan=2, sticky=tk.EW, pady=(10, 6))

        self.stitching_time_label.grid(row=8, column=0, columnspan=2, sticky=tk.W, pady=(0, 4))

        self.progress_frame_inner.grid(row=9, column=0, columnspan=2, sticky=tk.EW, pady=(0, 4))
    ...
    def _handle_result_message(self, message):
        if message[0] == "done":  self._handle_worker_success(message[1], message[2], message[3] if len(message) > 3 else None)
        elif message[0] == "error": self._handle_worker_error(message[1])

    def _handle_worker_success(self, output_path, stitching_time, seam_path=None):
        self.root.title("Eagle Eye Stitcher")
        self.stitching_time_label.config(text=f"⏱  拼接时间: {stitching_time:.2f}s")
        result = cv_imread(output_path)
        if result is not None:
            self.result_image = result
            if seam_path and os.path.exists(seam_path):
                self.seam_overlay_image = cv_imread(seam_path)
            else:
                self.seam_overlay_image = None
            self._display_current_result()
    ...
    def _cleanup_temp_result(self):
        temp_path = self.temp_result_path
        if temp_path and os.path.exists(temp_path):
            try: os.remove(temp_path)
            except OSError as e:
                logger.warning(f"无法删除临时结果文件 {temp_path}: {e}")
        self.temp_result_path = None
        if temp_path:
            from pathlib import Path as _Path
            seam_file = str(_Path(temp_path).with_stem(_Path(temp_path).stem + "_seam"))
            if os.path.exists(seam_file):
                try: os.remove(seam_file)
                except OSError as e:
                    logger.warning(f"无法删除临时缝合线文件 {seam_file}: {e}")

    def _display_current_result(self):
        if self.show_seam.get() and self.seam_overlay_image is not None:
            self._display_result(self.seam_overlay_image)
        else:
    ...
    def save_result(self):
        ...
        save_image = self.seam_overlay_image if (self.show_seam.get() and self.seam_overlay_image is not None) else self.result_image
        ...
        if file_path:
            try:
                if not cv_imwrite(file_path, save_image): raise RuntimeError("OpenCV 编码失败")
