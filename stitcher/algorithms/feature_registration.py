...
def registerTexture(
    ...
    ransac_thresholds = sorted(set([ransac_reproj_thresh, 4.0, 7.0]))
    last_exception = None
    ...
    for thresh in ransac_thresholds:
        ...
        if H0 is None:
            last_exception = RuntimeError(f"RANSAC failed (thresh={thresh:.1f})")
            logger.warning(f"RANSAC 变换估计失败，阈值={thresh:.1f}，尝试下一个阈值...")
            continue

        in1 = pts1[mask.ravel() == 1]
        in2 = pts2[mask.ravel() == 1]

        logger.info(f"Homography inliers: {len(in1)} (RANSAC thresh={thresh:.1f})")

        if len(in1) < 8:
            logger.warning(f"RANSAC 内点数不足 ({len(in1)} < 8)，阈值={thresh:.1f}，尝试放宽阈值...")
            continue
    ...
    raise RuntimeError("Too few RANSAC inliers (已尝试所有阈值: %s)" % ransac_thresholds)
